...
const TRUST_PILLS = [
  { icon: CheckCircle2, label: 'Direct Chair Booking' },
  { icon: ShieldCheck, label: 'Autoclave Sanitized Tools' },
  { icon: Users, label: '1-on-1 VIP Attention' },
];

function TeamMemberCard({ member }: { member: TeamMember }) {
  return (
    <div className="group flex flex-col justify-between overflow-hidden rounded-2xl md:rounded-3xl border border-[#E5E0D8] bg-white transition duration-300 hover:border-[#BA9D6A]/60 hover:shadow-xl dark:border-white/[0.08] dark:bg-[#141619] shadow-xs">
      ...
      <div className="p-5 flex-1 flex flex-col justify-between gap-4">
        ...
        <div className="pt-2">
          <Link
            ...
            className="gold-gradient inline-flex w-full items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[11px] font-bold uppercase tracking-wider text-[#0E1012] shadow-xs hover:brightness-105 transition-colors active:scale-95 cursor-pointer"
          >
            <Calendar size={13} />
        ...
        <div className="lg:col-span-5 space-y-3 text-left">
          ...
          <div className="flex flex-wrap items-center gap-2 pt-0.5">
            {TRUST_PILLS.map(({ icon: Icon, label }) => (
              <span
                key={label}
                className="inline-flex items-center gap-1 rounded-md bg-white px-2 py-0.5 text-[10px] font-semibold text-[#181A1C] shadow-2xs border border-[#E5E0D8] dark:border-white/10 dark:bg-white/[0.05] dark:text-white"
              >
                <Icon size={11} className="text-[#8C734B] dark:text-[#BA9D6A]" />
                <span>{label}</span>
              </span>
            ))}
          </div>
